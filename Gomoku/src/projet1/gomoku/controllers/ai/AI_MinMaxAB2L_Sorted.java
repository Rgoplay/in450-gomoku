...
public class AI_MinMaxAB2L_Sorted extends AIPlayer {
	...
	public AI_MinMaxAB2L_Sorted(int minimaxDepth, EvalFunction eval){
    ...
    private int minMax(GomokuBoard board, Player player, int depth, Player minMaxPlayer, int alpha, int beta) {
            ...
            for (currentCellCoords.column = 0; currentCellCoords.column < GomokuBoard.size; currentCellCoords.column++){
                if (boundaryBoard[currentCellCoords.column][currentCellCoords.row] > 0){ // Si la case est vide
                    if(depth > DEPTHSORT+1) {
	                    ...
	                    int tempValue = -new AI_MinMaxAB_2Limit(DEPTHSORT, eval).minMax(board,player,DEPTHSORT, inverseMinMaxPlayer,-beta, -alpha); // Evaluer le coup
        ...
        tab.sort((Pair a,Pair b) -> a.getScore().compareTo(b.getScore()));
    ...
    private Coords startMinMax(GomokuBoard board, Player player, int depth){ //scan les coups possible du tableau et les joue -> a utiliser récursivement dans le minmax
        ...
        for (currentCellCoords.row = 0; currentCellCoords.row < GomokuBoard.size; currentCellCoords.row++){
            for (currentCellCoords.column = 0; currentCellCoords.column < GomokuBoard.size; currentCellCoords.column++){
                if (boundaryBoard[currentCellCoords.column][currentCellCoords.row] > 0){ // Si la case est vide
                	 ...
                	 board.set(currentCellCoords, playerCellState); // Jouer le coup
                     addPlayableSquares(currentCellCoords.column, currentCellCoords.row);
                     int score = -new AI_MinMaxAB_2Limit(DEPTHSORT, eval).minMax(board,player,DEPTHSORT, inversePlayer,-beta, -alpha); // Evaluer le coup
                     board.set(currentCellCoords, TileState.Empty); // Annuler le coup
                     removePlayableSquares(currentCellCoords.column, currentCellCoords.row);
        ...
        for (Pair pair : tab) {
        	...
        	board.set(pair.getCoords(), playerCellState); // Jouer le coup
            addPlayableSquares(pair.getCoords().column, pair.getCoords().row);
            score = -minMax(board,player,depth-1, inversePlayer,-beta, -alpha); // Evaluer le coup
            board.set(pair.getCoords(), TileState.Empty); // Annuler le coup
            removePlayableSquares(pair.getCoords().column, pair.getCoords().row);
