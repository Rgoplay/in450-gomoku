 ...
 * et qui tri les coups avec un deuxième minmax pour augmenter le nombre de coupes
 */
public class AI_MinMaxAB2L_Sorted extends AIPlayer {
    ...
    int minMax_nonSorted(GomokuBoard board, Player player, int depth, Player minMaxPlayer, int alpha, int beta) {
    ...
    public Coords startMinMax(GomokuBoard board, Player player){
        ...
        for (Pair pair : tab) {
        	...
        	board.set(pair.getCoords(), playerCellState); // Jouer le coup
            addPlayableSquares(pair.getCoords().column, pair.getCoords().row);
            score = -minMax(board,player,depthMax-1, inversePlayer,-beta, -alpha); // Evaluer le coup
            board.set(pair.getCoords(), TileState.Empty); // Annuler le coup
            removePlayableSquares(pair.getCoords().column, pair.getCoords().row);
        ...
        return bestCoords; // Retourne les coordonnées du meilleur coup
    }
